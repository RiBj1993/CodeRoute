  ...
  var stringValue: String = "1"
    ...
    // Get user value
        ...
        if !( snapshot.value  is NSNull){
            ...
            self.Breponse = value?["reponse"] as? String ?? ""
            let IMAGE = value?["image"] as? String ?? ""
            print(choix1)
            print(choix2)
            print(choix3)
            print("le bon reponse", self.Breponse, " bbbbb")
            self.gameButtons[0].setTitle(choix1, for: .normal)
            self.gameButtons[1].setTitle(choix2, for: .normal)
            self.gameButtons[2].setTitle(choix3, for: .normal)
            self.gameButtons[3].setTitle(choix4, for: .normal)
            self.buttoi(VV: IMAGE)
            
            if(self.reponse != -1 && self.Breponse != "" ){
                if("\(self.reponse)" == self.Breponse ){
                    self.Myscore += 1
                    self.recentScoreLabel.text = "Recent: \(self.Myscore)"
                    print("dddddddddd")
                }
            }
...
// logoView.image = UIImage(named: "LaunchImage")
        case 3: // Emoji
        print("Emoji")
           self.reponse=4
